...
module.exports = function parsePdb(pdb) {
  ...
  pdbLines.forEach((pdbLine) => {
	...
	xloc: parseFloat(pdbLine.substring(30, 38)),
        yloc: parseFloat(pdbLine.substring(38, 46)),
        zloc: parseFloat(pdbLine.substring(46, 54)),
    ...
    } else if (pdbLine.substr(0, 6) === HETATOM_NAME) {
      hetatoms.push({
        serial: parseInt(pdbLine.substring(6, 11)),
        name: pdbLine.substring(12, 16).trim(),
        altLoc: pdbLine.substring(16, 17).trim(),
